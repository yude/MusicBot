...
public class PlaynextCmd extends DJCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyWarning("タイトルまたは URL を指定してください。");
            return;
        }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track)
        {
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) を追加しました。 "+(pos==0?"次に再生されます。":pos + " 番目に再生されます。"));
        ...
        {
            if(ytsearch)
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+"`"+event.getArgs()+"` に該当する項目はありませんでした。")).queue();
            else
        ...
        public void loadFailed(FriendlyException throwable)
        {
            if(throwable.severity==FriendlyException.Severity.COMMON)
                ...
                m.editMessage(event.getClient().getError()+" 項目を読み込めませんでした。").queue();
        }
    }
}
