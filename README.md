...
2. `data/config.txt` に [config.sample.yml](https://github.com/yude/MusicBot/blob/master/config.sample.txt) の内容をペーストし、適切な値を設定してください。
...
services:
  app:
    ...
    volumes:
      - "./data:/app"
```
4. Discord Developer Portal のボットの設定画面で、以下の項目を有効にします。
